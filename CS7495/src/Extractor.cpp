...
{
	cout << "Extractor::getTimeName(const std::string& filepath)" << endl;
	// Get file name
	char slash;
#ifdef _WIN32
	slash = '\\';
...
{
	cout << "Extractor::readGeoData(const std::string& filepath)" << endl;
	// Open text file
...
{
	cout << "Extractor::findFirstTimeStamp()" << endl;
	// Initialize to first
...
{
	cout << "Extractor::findCorrespondingGPSCoords()" << endl;
	// Time of frame
	local_date_time frameTime = *firstFrameTime;
	// For all frames
	int n = 0;
	...
	{
		// Initialize to first
		cout << "long min = std::abs( (*firstFrameTime - timestamps[0]).total_milliseconds() );" << endl;
		long min = std::abs( (*firstFrameTime - timestamps[0]).total_milliseconds() );
		cout << "unsigned int index = 0;" << endl;
		unsigned int index = 0;
		// Iterate through all coordinates/time stamps
		for (auto t : timestamps)
		{
